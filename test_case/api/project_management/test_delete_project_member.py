# -*- coding: utf-8 -*-
"""
@Time : 2022/6/29 15:20
@Author : name
@File : test_delete_project_member.py
"""
...
url = project_management.delete_project_member_url

@allure.epic("项目名称：Jugo接口自动化测试项目")
@allure.feature("模块名称：项目管理")
@allure.story("删除项目成员")
class TestDeleteProjectMember(object):
    @allure.title("未登录状态，删除项目成员")
    @allure.description("未登录状态，删除项目成员")
    def test_001_delete_project_member_without_login(self, get_base_info):
    ...
    @allure.title("必填项为空，删除项目成员")
    @allure.description("必填项为空，删除项目成员")
    def test_002_delete_project_member_without_requirement_param(self, get_base_info, get_token):
        base_url, request = get_base_info
    ...
    @allure.title("成功删除项目成员")
    @allure.description("成功删除项目成员")
    def test_003_delete_project_member_success(self):
        # 查询该用户下的项目
        ...
        delete_member_res = project_management.delete_project_member(re_query_project_res.json()["data"]["items"][0]["memberId"])
        logger.info("接口响应结果：{}".format(delete_member_res.json()))
        assert delete_member_res.json()["code"] == 10000
        assert delete_member_res.json()["msg"] == "成功"

    @allure.title("删除项目成员-当前用户下无权限操作的项目")
    @allure.description("删除项目成员-当前用户下无权限操作的项目")
    def test_004_delete_project_member_without_permission(self, handle_mysql):
        ...
        res = project_management.delete_project_member(query_project_result["id"])
        logger.info("接口响应结果：{}".format(res.json()))
